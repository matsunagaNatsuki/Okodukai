...
@section('content')
    ...
    <section class="profile-card">
        ...
        <form method="POST" action="{{ route('parent.profile.update') }}" enctype="multipart/form-data" data-loading data-loading-message="更新しています...">
            @csrf
            @method('PUT')

            <div class="form-group">
                <label class="form-label" for="name">名前 <span class="required-label">必須</span></label>
                <input class="form-control @error('name') is-invalid @enderror" id="name" name="name" type="text" value="{{ old('name', $parent->name) }}" maxlength="100" autocomplete="name" required>
