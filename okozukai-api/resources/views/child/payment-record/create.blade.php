...
@section('content')
    ...
    <section class="form-card">
        <form method="POST" action="{{ route('child.payment-record.store') }}" data-loading data-loading-message="登録しています...">
            @csrf
            <div class="form-group">
                <label class="form-label" for="title">使った内容 <span class="required-label">必須</span></label>
            ...
            <div class="form-group">
                <label class="form-label" for="amount">金額 <span class="required-label">必須</span></label>
            ...
            <div class="form-group">
                <label class="form-label" for="used_at">使用日 <span class="required-label">必須</span></label>
                ...
                @error('used_at')<p class="field-error">{{ $message }}</p>@enderror
            </div>
            <button class="button button--primary button--block" type="submit" @disabled($currentBalance < 1)>記録する</button>
            @if ($currentBalance < 1)<p class="field-error payment-balance-warning">残高がないため、支出を登録できません。</p>@endif
        </form>
    </section>
@endsection
